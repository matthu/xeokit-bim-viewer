...
class SectionTool extends Controller { // XX
    ...
    _counterElement: HTMLElement;
    _menuButtonElement: Element;
    _menuButtonArrowElement: Element;

    _sectionPlanesPlugin: SectionPlanesPlugin;
    _sectionToolContextMenu: SectionToolContextMenu;
    ...
    constructor(parent: Controller, cfg: BIMConfig) {
        ...
        this._buttonElement.addEventListener("click", (e: any) => {
            if (!this.getEnabled()) {
                return;
            }
        ...
        document.addEventListener("mousedown", (e: any) => {
            ...
            if ((e.target as Element).classList.contains("xeokit-context-menu-item")) {
                // Allow click on menu item
                return;
            }

            if (e.target === this._menuButtonElement || (e.target as any).parentNode === this._menuButtonElement) {
                e.preventDefault();
    ...
    _initSectionMode() {
        ...
        this.viewer.scene.input.on("mouseclicked", (coords: number[]) => {
            ...
            if (pickResult) {
                ...
                this._sectionPlanesPlugin.showControl(sectionPlane.id as string);
